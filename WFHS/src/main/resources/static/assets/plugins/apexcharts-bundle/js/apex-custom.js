$(function () {
	...
	//Top 3 division
	$.ajax({
	    url: '/update-chart-division', // Update the URL to the endpoint for top 3 departments
	    type: 'GET',
	    success: function(data) {
	        // Process the data received from the backend
	        var sortedData = Object.entries(data).sort((a, b) => b[1] - a[1]); // Sort data by counts in descending order
	        
	        // Select top 3 entries
	        var top3Data = sortedData.slice(0, 3);
	        
	        // Extract department names and counts
	        var divisionNames = top3Data.map(entry => entry[0]);
	        var counts = top3Data.map(entry => entry[1]);
	        ...
	        var chart = new ApexCharts(document.querySelector("#chartDivision"), options);
	        chart.render();
	    },
	...
	//Top 3 department
	$.ajax({
	    url: '/update-chart-department', // Update the URL to the endpoint for top 3 departments
	    type: 'GET',
	    success: function(data) {
	        // Process the data received from the backend
	        var sortedData = Object.entries(data).sort((a, b) => b[1] - a[1]); // Sort data by counts in descending order
	        ...
	        // Extract department names and counts
	        var departmentNames = top3Data.map(entry => entry[0]);
	        var counts = top3Data.map(entry => entry[1]);
	        ...
	        // Re-render the chart with updated options
	        var chart = new ApexCharts(document.querySelector("#chartDepartment"), options);
	        chart.render();
	    },
	    error: function(xhr, status, error) {
	        console.error('Error updating chart:', error);
	 ...
	 //Top 3 teams   
	$.ajax({
        url: '/update-chart-team',
        type: 'GET',
        success: function(data) {
            // Process the data received from the backend
            var sortedData = Object.entries(data).sort((a, b) => b[1] - a[1]); // Sort data by percentages
            
            // Select top 3 entries
            var top3Data = sortedData.slice(0, 3);
            
            // Extract team names and percentages
            var teamNames = top3Data.map(entry => entry[0]);
            var percentages = top3Data.map(entry => entry[1]);
            ...
            // Re-render the chart with updated options
            var chart = new ApexCharts(document.querySelector("#chartTeam"), options);
            chart.render();
        },
        error: function(xhr, status, error) {
            console.error('Error updating chart:', error);
